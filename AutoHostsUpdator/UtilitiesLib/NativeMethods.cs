...
namespace UtilitiesLib
{
    public class NativeMethods
    {
        ...
        public static WindowsImpersonationContext GetImpersonationContext(string userName, string domainName, string password)
        {
            ...
            const int logon32LogonNewCredentials = 5;
            //const int LOGON32_LOGON_INTERACTIVE = 2;

            try
            {
                SafeTokenHandle safeTokenHandle;
                var returnValue = LogonUser(userName, domainName, password, logon32LogonNewCredentials, logon32ProviderDefault, out safeTokenHandle);
