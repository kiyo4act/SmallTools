...
namespace AutoHostsUpdateService
{
    public class EditHosts
    {
        ...
        {
            _eventLog = ev;
            _eventLog.Source = LocalResources.MGstrEventLogSource;
            ...
            else
            {
                ...
                foreach (string readLine in readLinesCollection)
                {
                    ...
                    foreach (string importLine in importLines)
                    {
                        ...
                        if (readLine.IndexOf(searchWord, StringComparison.Ordinal) != -1)
                        {
                            // 読み出した行はバッファーに入れない
                    ...
                    if (!fIsRedundancy)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                buffer += Environment.NewLine + readLine;
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// 指定した文字列以外の文字列をマージして書き出す
        /// </summary>
        /// <param name="importStrings"></param>
        /// <param name="separate"></param>
        /// <param name="searchNum"></param>
        /// <param name="exceptStrings"></param>
        /// <returns></returns>
        public bool MargeString(string importStrings, char separate, int searchNum, string exceptStrings)
        {
            string sendStrings = string.Empty;
            string[] importLines = importStrings.Split('\n');

            foreach (string importLine in importLines)
            {
                if (importLine.IndexOf(exceptStrings, StringComparison.Ordinal) == -1)
                {
                    //eventLog.WriteEntry(importLine+" cannot find "+ exceptStrings, EventLogEntryType.Information);
                    sendStrings += importLine;

                    // 末尾に\rが残っていた場合
                    if (sendStrings.EndsWith("\r"))
                    {
                        sendStrings += "\n";
                    }
                    else
                    {
                        sendStrings += Environment.NewLine;
                    }
                }
            }

            // 末尾の改行を削除する
            sendStrings = System.Text.RegularExpressions.Regex.Replace(sendStrings, @"[\r\n]+$", "");
            //eventLog.WriteEntry("MargeString(" + sendStrings + ", ' ', 1);", EventLogEntryType.Information);
            bool fResult = MargeString(sendStrings, separate, searchNum);
